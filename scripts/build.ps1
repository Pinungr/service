...
& "$projectRoot\.venv\Scripts\python.exe" -m PyInstaller --noconfirm --clean --distpath $OutputDirectory RepairShopManager.spec
if ($LASTEXITCODE -ne 0) { throw 'Windows build failed.' }
$applicationFile = Join-Path $OutputDirectory 'RepairShopManager.exe'
